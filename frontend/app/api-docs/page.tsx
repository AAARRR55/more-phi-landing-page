  ...
  return (
    <main className="min-h-screen bg-background px-5 py-10 text-foreground">
      <div className="mx-auto max-w-6xl">
        <Link href="/" className="text-sm text-cyan hover:underline" data-testid="api-docs-home-link">Back to More-Phi</Link>
        <section className="mt-10 border border-slate-line bg-card/70 p-8" data-testid="api-docs-hero">
          ...
          <p className="mt-5 max-w-3xl text-sm leading-7 text-muted-foreground">
            FastAPI + MongoDB backend with JWT accounts, Stripe Checkout, webhook reconciliation, license delivery hooks, payment-event logging, activation tracking, and rate limits on sensitive endpoints.
          </p>
        </section>

        <section className="mt-8 grid border border-slate-line" data-testid="api-docs-endpoints">
          {endpoints.map(([method, path, description]) => (
            <div key={path} className="grid gap-3 border-b border-slate-line p-4 last:border-b-0 md:grid-cols-[100px_1fr_1.4fr]" data-testid={`api-docs-endpoint-${path.replaceAll('/', '-').replace(/[{}]/g, '')}`}>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-2">
          <div className="border border-slate-line bg-card/70 p-6" data-testid="api-docs-register-example">
            ...
            <pre className="mt-4 overflow-x-auto bg-black/40 p-4 font-mono text-xs text-muted-foreground">{`POST /api/auth/register
{
  ...
  "country": "US"
          ...
          <div className="border border-slate-line bg-card/70 p-6" data-testid="api-docs-checkout-example">
